#!/bin/bash
# test_logger_compat.sh - Comprehensive logging utility for test suite
# Compatible version for older bash versions
...
# Test summary data - use flat variables instead of associative arrays
TEST_COUNT=0
PASS_COUNT=0
FAIL_COUNT=0
WARN_COUNT=0
...
test_start() {
    ...
    if [[ $TEST_COUNT -gt 1 ]]; then
        ...
        truncate -s-1 "$JSON_SUMMARY_FILE" 2>/dev/null || sed -i '' -e '$ d' "$JSON_SUMMARY_FILE" 
        echo "," >> "$JSON_SUMMARY_FILE"
    fi
...
generate_summary() {
    ...
    local start_time=$(grep "start_time" "$JSON_SUMMARY_FILE" | head -1 | cut -d'"' -f4)
    local start_seconds=$(date -j -f "%Y-%m-%d %H:%M:%S" "$start_time" +%s 2>/dev/null || date +%s)
    local total_duration=$(($(date +%s) - start_seconds))
...
EOF
    ...
    find "$LOG_DIR" -type f -name "*.json" -mtime +$LOG_RETENTION_DAYS -delete 2>/dev/null || true
}

# Register cleanup for graceful termination
cleanup() {
    # If any tests were started but not completed, mark them as failed
    if [[ -n "$CURRENT_LOG_FILE" && -n "$CURRENT_TEST_ID" ]]; then
        test_end "$CURRENT_TEST_ID" "fail" "Test was interrupted"
    fi
